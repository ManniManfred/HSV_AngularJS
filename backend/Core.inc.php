		...
		foreach ($data["player"] as $player) {
			if (array_key_exists($player["id"], $playerToMatchPlayer)) {
				...
				if ($querySql != "" && $stmt = $this->db->prepare($querySql)) {
					$stmt->bind_param("iiiii", $player["hasRedCard"], $player["hasYellowCard"], $player["hasYellowRedCard"], $player["goals"], $matchPlayerId);
					$stmt->execute();
					$stmt->close();
				}
			} else {
				...
				$querySql .= "VALUES (?, ?, ?, ?, ?, ?)";
				if ($stmt = $this->db->prepare($querySql)) {
					$stmt->bind_param("iiiiii", $data["matchId"], $saisonPlayerId, $player["hasRedCard"], $player["hasYellowCard"], $player["hasYellowRedCard"], $player["goals"]);
